...
class UserProfileHeader: UICollectionViewCell {
    ...
    fileprivate func setupEditFollowButton() {
        ...
        if currentLoggedInUserId == userId {
            editProfileFollowButton.setTitle("Edit Profile", for: UIControl.State.normal)
        } else {
    ...
    @objc func handleEditProfileOrFollow() {
        ...
        if editProfileFollowButton.titleLabel?.text == "Unfollow" {
            ...
            Database.database().reference().child("following").child(currentLoggedInUserId).child(userId).removeValue(completionBlock: { (error, ref) in
                if let error = error {
                    print("******************************************************")
                    print(" <UnFollow User 실패> \n\t", error)
                    print("******************************************************")
                    return
                }
                
                print("******************************************************")
                print(" <Follow User 성공> \n\t", self.user?.username ?? "")
                print("******************************************************")
    ...
    let bookmarkButton: UIButton = {
        let button = UIButton(type: .system)
        button.setImage(UIImage(named: "ribbon"), for: UIControl.State.normal)
        button.tintColor = UIColor(white: 0, alpha: 0.2)
        return button
    }()
